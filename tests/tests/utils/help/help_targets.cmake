...
function(${${TEST_NAME}})
    ...
    set_target_properties(
        build_bootloader
        PROPERTIES DESCRIPTION "Build first-stage bootloader (ROM code)"
    )

    add_custom_target(build_firmware)
    set_target_properties(
        build_firmware
        PROPERTIES DESCRIPTION "Build application firmware for main processor"
    )

    add_custom_target(build_fpga_bitstream)
    set_target_properties(
        build_fpga_bitstream
        PROPERTIES DESCRIPTION "Synthesize and generate FPGA bitstream"
    )
    ...
    set_target_properties(
        sim_core_unit_tests
        PROPERTIES DESCRIPTION "Run CPU core unit tests in Verilator"
    )

    add_custom_target(sim_peripheral_tests)
    set_target_properties(
        sim_peripheral_tests
        PROPERTIES DESCRIPTION "Run peripheral subsystem tests"
    )

    add_custom_target(sim_integration)
    set_target_properties(
        sim_integration
        PROPERTIES DESCRIPTION "Run full SoC integration simulation"
    )
    ...
    set_target_properties(
        flash_bootloader
        PROPERTIES DESCRIPTION "Program bootloader to on-chip ROM/Flash"
    )

    add_custom_target(flash_firmware)
    set_target_properties(
        flash_firmware
        PROPERTIES DESCRIPTION "Program application firmware to external Flash"
    )

    add_custom_target(flash_fpga)
    set_target_properties(
        flash_fpga
        PROPERTIES DESCRIPTION "Program FPGA configuration memory"
    )
    ...
    function(ct_json_find_item JSON_FILE ARRAY_NAME ITEM_NAME OUT_INDEX)
        ...
        if(err)
            message(
                FATAL_ERROR
                "Failed to read array '${ARRAY_NAME}' from ${JSON_FILE}: ${err}"
            )
        endif()
    ...
    function(ct_assert_item_in_json JSON_FILE ARRAY_NAME ITEM_NAME)
        ...
        if(index EQUAL -1)
            message(
                FATAL_ERROR
                "Item '${ITEM_NAME}' not found in ${ARRAY_NAME} of ${JSON_FILE}"
            )
        else()
    ...
    function(ct_assert_item_not_in_json JSON_FILE ARRAY_NAME ITEM_NAME)
        ...
        if(NOT index EQUAL -1)
            message(
                FATAL_ERROR
                "Item '${ITEM_NAME}' should NOT be in ${ARRAY_NAME} of ${JSON_FILE}"
            )
        else()
            message(
                STATUS
                "✓ Item '${ITEM_NAME}' correctly excluded from ${JSON_FILE}"
            )
        endif()
    endfunction()
    ...
    function(ct_assert_item_has_group JSON_FILE ARRAY_NAME ITEM_NAME GROUP_NAME)
        ...
        if(err)
            message(
                FATAL_ERROR
                "Failed to read groups for '${ITEM_NAME}': ${err}"
            )
        endif()
        ...
        if(NOT found)
            message(
                FATAL_ERROR
                "Item '${ITEM_NAME}' does not have group '${GROUP_NAME}'"
            )
        else()
    ...
    function(
        ct_assert_item_has_description
        JSON_FILE
        ARRAY_NAME
        ITEM_NAME
        EXPECTED_DESC
        ...
        if(err)
            message(
                FATAL_ERROR
                "Failed to read description for '${ITEM_NAME}': ${err}"
            )
        endif()
        ...
        if(pos EQUAL -1)
            message(
                FATAL_ERROR
                "Description '${EXPECTED_DESC}' not found in item '${ITEM_NAME}'. Got: ${description}"
            )
        else()
    ...
    if(NOT ddr_index EQUAL -1)
        ...
        if(found_peripheral_group)
            message(
                FATAL_ERROR
                "Target 'run_test_ddr_calibration' should NOT have group 'all_peripheral_tests'"
            )
        else()
            message(
                STATUS
                "✓ Target 'run_test_ddr_calibration' correctly excluded from 'all_peripheral_tests' group"
            )
        endif()
    endif()
    ...
    if(NOT fpu_index EQUAL -1)
        ...
        if(NOT opt_type STREQUAL "Boolean")
            message(
                FATAL_ERROR
                "ENABLE_FPU should have type 'Boolean', got '${opt_type}'"
            )
        endif()
        if(NOT opt_current STREQUAL "ON")
            message(
                FATAL_ERROR
                "ENABLE_FPU current value should be 'ON', got '${opt_current}'"
            )
        endif()
        if(NOT opt_default STREQUAL "ON")
            message(
                FATAL_ERROR
                "ENABLE_FPU default value should be 'ON', got '${opt_default}'"
            )
        endif()
