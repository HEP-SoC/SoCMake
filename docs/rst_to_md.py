...
def get_rst_title(lines: list) -> str | None:
    ...
    for i, line in enumerate(lines):
        if is_underline(line) and i > 0 and lines[i - 1].strip() and not is_underline(lines[i - 1]):
...
def parse_params(lines: list) -> tuple:
    ...
    def to_list(d: dict) -> list:
        return [{'name': k, 'type': v.get('type', ''), 'desc': v.get('desc', '')}
                for k, v in d.items()]
...
def rst_file_to_md(rst_text: str, module_name: str, sidebar_position: int) -> str:
    ...
    func_starts = [i for i, l in enumerate(lines) if l.strip().startswith('.. function::')]
...
def process_dir(rst_dir: Path, md_dir: Path, is_root: bool = True, position: int = 1) -> None:
    ...
    if is_root:
        ...
        (md_dir / '_category_.json').write_text(json.dumps(category, indent=2) + '\n', 'utf-8')
