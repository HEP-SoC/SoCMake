...
To give the right flags to use the DPI-C with the simulator used, the `${simulator}_configure_cxx()` function can be used, in our case, we will use `verilator_configure_cxx(LIBRARIES DPI-C)`.
