...
Executing the `adder` target will compile a [static library](https://en.wikipedia.org/wiki/Static_library) of the IP block.<br/>
This time we are not asking Verilator to create a `main.cpp` file as we will write it ourselves.<br/>
We are also passing `TRACE` argument to `verilator()`, indicating that we want to enable [`VCD`](https://en.wikipedia.org/wiki/Value_change_dump) generation in Verilated model.
...
It's important to not forget to add the `::vlt` after the name of the Verilated static library, it will allows a proper linking. `__vlt` works as well they are alias of each others.
