...
function(add_cocotb_tests IP_LIB DIRECTORY)
    include(
        "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/../../utils/subdirectory_search.cmake"
    )
    ...
    foreach(test ${TEST_SUBDIRS})
        ...
        if(NOT COCOTB_TESTCASE)
            ...
            add_test(NAME ${COCOTB_MODULE} COMMAND ${COCOTB_SIM_RUN_CMD})
            # Set environment variables for the test (cannot be set as for the custom command)
            set_property(
                TEST ${COCOTB_MODULE}
                APPEND
                PROPERTY ENVIRONMENT PYTHONPATH=${COCOTB_PYTHONPATH}
            )
            set_property(
                TEST ${COCOTB_MODULE}
                APPEND
                PROPERTY ENVIRONMENT MODULE=${COCOTB_MODULE}
            )
            set_property(
                TEST ${COCOTB_MODULE}
                APPEND
                PROPERTY
                    ENVIRONMENT
                        COCOTB_RESULTS_FILE=${PROJECT_BINARY_DIR}/results.xml
            )
            ...
            set_property(
                TEST ${COCOTB_MODULE}
                PROPERTY PASS_REGULAR_EXPRESSION "[^a-z]FAIL=0"
            )
            ...
            foreach(i RANGE 1 ${COCOTB_TESTCASE})
                ...
                if(COCOTB_PYTHONPATH)
                    set_property(
                        TEST ${COCOTB_MODULE}_test_${test_num}
                        APPEND
                        PROPERTY ENVIRONMENT PYTHONPATH=${COCOTB_PYTHONPATH}
                    )
                endif()
                ...
                set_property(
                    ...
                    PROPERTY
                        ENVIRONMENT
                            COCOTB_RESULTS_FILE=${PROJECT_BINARY_DIR}/test_${test_num}/results.xml
                )
                set_property(
                    TEST ${COCOTB_MODULE}_test_${test_num}
                    APPEND
                    PROPERTY
                        ENVIRONMENT TESTCASE=${COCOTB_MODULE}_test_${test_num}
                )
                # Simulators (e.g., icarus, xcelium) always returns 0 (pass) so check the output to detect a failure
                # We only check if test passed or not checking cocotb summary header at the end
                set_property(
