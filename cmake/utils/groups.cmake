...
function(group_custom_options GROUP_NAME)
    cmake_parse_arguments(ARG "" "PATTERN" "OPTION_LIST" ${ARGN})
    if(ARG_UNPARSED_ARGUMENTS)
        message(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION} passed unrecognized argument " "${ARG_UNPARSED_ARGUMENTS}")
    endif()

    if(ARG_PATTERN AND ARG_OPTION_LIST)
        message(FATAL_ERROR "Arguments PATTERN and OPTION_LIST cannot be used at the same time")
    endif()

    if(ARG_PATTERN)
        get_property(all_options GLOBAL PROPERTY SOCMAKE_OPTIONS)
        list(FILTER all_options INCLUDE REGEX "${ARG_PATTERN}")
        set(options ${all_options})
    elseif(ARG_OPTION_LIST)
        set(options ${ARG_OPTION_LIST})
    else()
        message(FATAL_ERROR "Specify either PATTERN or OPTION_LIST arguments")
    endif()

    if(NOT options)
        message(WARNING "No options found for PATTERN: ${ARG_PATTERN} or OPTION_LIST: ${ARG_OPTION_LIST}")
        return()
    endif()

    foreach(option ${options})
        set_property(GLOBAL PROPERTY SOCMAKE_${option}_SOCMAKE_GROUPS "${GROUP_NAME}")
    endforeach()
endfunction()
